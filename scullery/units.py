...
_prefixes = {"n": 10**-9, "u": 10**-6, "m": 0.001, "k": 1000,
...
define_unit("degF", (lambda x: (x+459.67) * (5/9),
                     lambda x: (x*(9/5))-459.67), "temperature")
...
def convert(v, fromUnit, toUnit):
    if fromUnit == toUnit:
        return v
    try:
        fr, frmul = parse_prefix(fromUnit)
        to, tomul = parse_prefix(toUnit)
...
def si_format_number(number, digits=2):
    if number == 0:
        return "0"
    if number > 10**15:
